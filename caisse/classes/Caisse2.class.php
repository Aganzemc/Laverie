...
<?php 
	class Caisse2 extends Dbh{
		...
		public function getMonthly(){
			...
			// $query = $this->connect_db->prepare($sql1);
			// $query->execute();

			// $sql2 = "

			// 	SELECT * FROM monthly_repport 
			// ";

			$sql3 = "
				SELECT 
					SUM(p.montant) AS 'intput',
					SUM(d.montant) AS 'output',
					MONTH(p.payment_date) AS 'month',
					YEAR(p.payment_date) AS 'year'
				FROM 
					depenses d, payments p
				WHERE MONTH(d.date_reg) = MONTH(p.payment_date) AND YEAR(d.date_reg) = YEAR(p.payment_date)
